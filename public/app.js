document.addEventListener('DOMContentLoaded', () => {
    ...
    btnGenerateScript.addEventListener('click', () => {
        ...
        var scriptParts = [
            '// ==========================================',
            '// PART 1: AUTO ASSIGN DESCRIPTIONS',
            '// ==========================================',
            'const descriptions = [',
            arrayLines,
            '];',
            ...
            '});',
            '',
            'console.log(`\u2705 Successfully pushed ${Math.min(descriptions.length, textAreas.length)} descriptions.`);',
            '',
            '// ==========================================',
            '// PART 2: AUTO SCHEDULE (10 MIN INTERVALS)',
            '// ==========================================',
            'const labels = Array.from(document.querySelectorAll(\'label, span, div\')).filter(el => el.innerText && el.innerText.trim() === \'Schedule\');',
            'labels.forEach(label => label.click());',
            '',
            'console.log(`\u23f3 Attempted to click Schedule for ${labels.length} items. Waiting for UI...`);',
            '',
            '// Wait 1 second for the UI to update, then fill the times',
            'setTimeout(() => {',
            '    const timeInputs = Array.from(document.querySelectorAll(\'input[type="text"]\')).filter(input => ',
            '        /^\\d{1,2} [A-Za-z]{3} \\d{4} \\d{2}:\\d{2} [AP]M$/.test(input.value)',
            '    );',
            '    ',
            '    let scheduledTime = new Date();',
            '',
            '    timeInputs.forEach((input, index) => {',
            '        // Add 10 minutes for each video',
            '        scheduledTime.setMinutes(scheduledTime.getMinutes() + 10);',
            '',
            '        // Format to match the site\'s exact format: "DD Mmm YYYY hh:mm A"',
            '        let day = scheduledTime.getDate().toString().padStart(2, \'0\');',
            '        let month = scheduledTime.toLocaleString(\'en-US\', { month: \'short\' });',
            '        let year = scheduledTime.getFullYear();',
            '        ',
            '        let hours = scheduledTime.getHours();',
            '        let minutes = scheduledTime.getMinutes().toString().padStart(2, \'0\');',
            ...
            '        ',
            '        const formattedTime = `${day} ${month} ${year} ${strHours}:${minutes} ${ampm}`;',
            '',
            '        // Set the value',
            ...
            '    });',
            '',
            '    console.log(`\u2705 Successfully scheduled ${timeInputs.length} videos, spaced 10 minutes apart.`);',
            '}, 1000);'
        ];
